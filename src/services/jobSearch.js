...
export function searchJobs(params) {
    const { query, location, remote, experience, } = params;
    const normalizedQuery = query.toLowerCase();
    return jobs.filter((job) => {
        const queryMatch = job.title.toLowerCase().includes(normalizedQuery) ||
            job.company.toLowerCase().includes(normalizedQuery) ||
            job.skills.some((skill) => skill.toLowerCase().includes(normalizedQuery));
        const locationMatch = location
            ? job.location.toLowerCase().includes(location.toLowerCase())
            : true;
        const remoteMatch = remote !== undefined ? job.remote === remote : true;
...
//# sourceMappingURL=jobSearch.js.map